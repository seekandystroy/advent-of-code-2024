...
module Day10
  ...
  def self.trailhead_score(matrix, row, col)
    ...
    dfs_score(deep_copy_matrix(matrix), row, col)
  end

  def self.dfs_score(matrix, row, col)
    ...
    up = row > 0 && matrix[row - 1][col] == height + 1 ?                    dfs_score(matrix, row - 1, col) : 0
    down = row < matrix.length - 1 && matrix[row + 1][col] == height + 1 ?  dfs_score(matrix, row + 1, col) : 0
    right = col < matrix.length - 1 && matrix[row][col + 1] == height + 1 ? dfs_score(matrix, row, col + 1) : 0
    left = col > 0 && matrix[row][col - 1] == height + 1 ?                  dfs_score(matrix, row, col - 1) : 0
  ...
  private_class_method(:dfs_score)
